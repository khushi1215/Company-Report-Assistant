...
### Why this project was not deployed live
...
**Chosen:** No live deployment for this version. The app runs fully, correctly, and is demoed locally.
**Reasoning:** Real memory usage was measured directly, not estimated, using Windows process monitoring, roughly 941MB for the app itself (embeddings model, Chroma, Streamlit, LangChain) plus roughly 2GB more if running the LLM locally via Ollama. That ruled out every free tier checked. Switching the deployed LLM to Groq's free API would have solved the LLM's memory cost, but the app's own footprint alone still exceeds free-tier limits everywhere, meaning even with that swap, a paid tier (at least $25/month) would still be required. Weighed against the ongoing monthly cost, and after already investing significant, real effort in diagnosing and fixing retrieval accuracy (see Discoveries and Changelog), the decision was to stop and document this clearly rather than add more moving pieces under time pressure. Every part of the actual RAG pipeline, chunking, embedding, hybrid retrieval, evaluation, still runs and can be demoed locally or from a screen recording. This was a deliberate, evidence-based decision, not an unfinished task, real infrastructure constraints were measured, real costs were weighed, and a clear line was drawn.
...
- **Deployment investigated and deliberately not pursued.** Measured real memory requirements directly (Windows process monitoring during actual use), checked real numbers against Render's tiers and Streamlit Community Cloud's limits, and evaluated switching the deployed LLM to a free hosted API (Groq) as a way to reduce cost. Concluded that even with that swap, the app's own memory footprint requires a paid hosting tier (at minimum $25/month), and made the deliberate decision to stop there rather than keep adding infrastructure changes under time pressure. The full RAG pipeline is complete, correct, and demoed locally. This project's v1 is considered feature-complete, with deployment marked as a known, reasoned, and clearly documented decision rather than an unfinished task.

---

## Still open

- **BM25 keyword search still misses some content that vector search alone can now catch.** The Sun Pharma US business question was fixed by the embedding model switch, but the underlying BM25 weakness (matching "%" against the word "percentage" fails) is unresolved and would need query expansion or synonym handling to fix properly. Not urgent, since the embedding model upgrade is now doing most of the real work in the hybrid combination.
- Docker + Render deployment, evaluated and deliberately not pursued for this version (see Tech Stack section above for the real reasoning and measured numbers behind that decision)
- Chunk size and overlap settings, still the original starting values, not revisited since the embedding model change, worth a future check now that retrieval quality has genuinely improved
